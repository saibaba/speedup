...
function findSchema(conn, objtype, callback) {
...
function findBeforePOST(conn, objtype, callback) {
    conn.db.collection("metadata", function(err, collection) {
        if (!err) {
            var qry = { objtype : objtype, type: "trigger", name: "beforePOST" };
            collection.findOne(qry, function(err, item) {
                if (!err)  {
                    var script = null;
                    if (item) {
                        var scriptSrc = item.body;
                        script = vm.createScript(scriptSrc, "beforePOST");
                    }
...
var inserter = function(conn, objtype, schema, body, callback) {
    ...
    try {
        ...
        findBeforePOST(conn, objtype, function(err, script) {
            if (!err) {
                if (script) {
                    var sandbox = {
                        objtype: objtype,
